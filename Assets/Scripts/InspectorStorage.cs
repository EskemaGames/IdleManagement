...
public class InspectorStorage : EG_Singleton<InspectorStorage>
{
    ...
    #if UNITY_EDITOR
            #region editor auto filling categories
            
//             [ContextMenu("Fill tiles prefabs")]
//             public void LoadAllTiles()
//             {
//                 var subfolders = Directory.GetDirectories(Application.dataPath + "/GameAssets/Prefabs/Map/");
//
//                 int totalFiles = 0;
//                 int counter = 0;
//
...
//                 {
//                     var assetFiles = System.IO.Directory.GetFiles(folder + "/");
//
//                     var di = new DirectoryInfo(folder);
//                     var files = di.GetFiles("*.prefab");
//                     totalFiles += files.Length;
//                 }
//
//                 tilesPrefabs = null;
//                 tilesPrefabs = new PrefabObjects[totalFiles];
//                 
//                 foreach (var folder in subfolders)
//                 {
//                     var assetFiles = System.IO.Directory.GetFiles(folder + "/");
...
//                     {
// #if UNITY_EDITOR_WIN
//                         var assetPath = "Assets" + asset.Replace('\\', '/').Replace(Application.dataPath, "");
// #elif UNITY_EDITOR_LINUX || UNITY_EDITOR_OSX
//                         var assetPath = "Assets" + asset.Replace(Application.dataPath, "").Replace('\\', '/');
// #endif
//                         
//                         var s = (GameObject) UnityEditor.AssetDatabase.LoadAssetAtPath(assetPath, typeof(GameObject));
//                         if (s != null)
//                         {
//                             tilesPrefabs[counter].prefab = s;
//                             tilesPrefabs[counter].NameType = s.name;
//                             ++counter;
//                         }
//                     }
//                 }
//             }
            ...
            [ContextMenu("Fill character prefabs")]
            public void LoadAllCharacters()
            {
                var subfolders = Directory.GetDirectories(Application.dataPath + "/GameAssets/Prefabs/Entities/");
                ...
                characterPrefabs = new PrefabObjects[totalFiles];
                ...
                foreach (var folder in subfolders)
                {
                    ...
                    foreach (var asset in assetFiles)
                    {
                        ...
                        if (s != null)
                        {
                            characterPrefabs[counter].prefab = s;
                            characterPrefabs[counter].NameType = s.name;
                            ++counter;
                        }
                    }
                }
